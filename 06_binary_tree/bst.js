...
class BinarySearchTree {
  ...
  deleteNode(currentRoot, key) {
    ...
    if (key < currentRoot.key) {
      ...
      if (!currentRoot.left && !currentRoot.right) {  // if node to be deleted is leaf
        return null
      } else if (!node.left) {  // if node to be deleted have 1 child only
        return node.right
      } else if (!node.right) {  // if node to be deleted have 1 child only
        return node.left
      } else {      // replace node with the smallest value in right subtree(in order successor)
